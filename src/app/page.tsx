    ...
    <main className="relative min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        <section className="relative py-32 px-6 overflow-hidden">
          {/* 質感のある背景: ダークグレーから黒へのグラデーション + 微細なメッシュ */}
          <div className="absolute inset-0 bg-gradient-to-br from-zinc-900 via-black to-zinc-950 -z-10" />
          <div className="absolute inset-0 opacity-[0.1] pointer-events-none -z-10" style={{ backgroundImage: `linear-gradient(rgba(255,255,255,.05) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.05) 1px, transparent 1px)`, backgroundSize: '4px 4px' }} />

          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 items-center text-white">
            ...
            <div className="md:col-span-4 space-y-8">
              ...
              <div className="pt-8 border-t border-zinc-800">
                <h3 className="text-xl font-bold mb-8 opacity-90">{latestVideo?.title || "Latest Release"}</h3>
                <Link href="/video" className="inline-block bg-white text-black px-10 py-4 text-[10px] font-black tracking-widest uppercase hover:bg-red-600 hover:text-white transition-all">Watch More</Link>
              </div>
            </div>
          </div>
        </section>
